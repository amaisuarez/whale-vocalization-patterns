...
**Phase 1 — Replication (done).** Before searching for new structure, the pipeline is validated by testing whether unsupervised clustering recovers the coda types already defined by experts for the EC1 clan of Dominica.
...
**Phase 2 — Tempo across the repertoire (done).** See [`notebooks/03_tempo_across_repertoire.ipynb`](notebooks/03_tempo_across_repertoire.ipynb).

- With stricter safeguards (Ashman's D and a *session-level* bootstrap), `1+1+3` remains the only coda type with robust, discrete tempo classes. Apparent splits in four other types turned out to be artefacts.
- Rhythm and tempo are not independent: coda shape changes slightly with speed.
- Candidate findings (not established): tempo multimodality in the `i` types; `7D1` sharing two tempo classes with `1+1+3` (confounded by one social unit).
- Not supported: tempo shifting together across coda types within a recording session.

**Next:** model how rhythm changes with tempo (the link to *rubato*).
...
│   └── 03_tempo_across_repertoire.ipynb
